...
{
	private IAssociationRegistry<string, string> _categoryRegistry;

	private void Start()
	{
		// Creating AssociationRegistry with capacity strategy
		var capacityStrategy = new CapacityStrategy();
		_categoryRegistry = new SampleAssociationRegistry(capacityStrategy, capacity: 5);
	...
	private sealed class SampleAssociationRegistry : AssociationRegistry<string, string>
	{
		private readonly ICapacityStrategy _capacityStrategy;

		public SampleAssociationRegistry(ICapacityStrategy capacityStrategy, int capacity) : base(capacityStrategy, capacity)
		{
			_capacityStrategy = capacityStrategy;
		}

		protected override IKeyAssociation<string, string> GetRawBinding(string key)
		{
			return new KeyAssociation<string, string>(_capacityStrategy, key);
		}
	}
}
